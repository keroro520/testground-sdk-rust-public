...
impl RunParams {
    pub fn new(kvs: &HashMap<String, String>) -> Self {
        ...
        RunParams {
            ...
            test_instance_count: kvs.get(ENV_TEST_INSTANCE_COUNT).and_then(|raw| raw.parse::<u64>().ok()).unwrap(),
            test_instance_role: kvs.get(ENV_TEST_INSTANCE_ROLE).cloned().unwrap_or_default(),
            test_instance_params: kvs.get(ENV_TEST_INSTANCE_PARAMS).map( unpack_params).unwrap_or_default(),
            test_group_id: kvs.get(ENV_TEST_GROUP_ID).cloned().unwrap_or_default(),
            test_group_instance_count: kvs.get(ENV_TEST_GROUP_INSTANCE_COUNT).and_then(|raw| raw.parse::<u64>().ok()).unwrap_or_default(),
            test_sidecar: kvs.get(ENV_TEST_SIDECAR).cloned().and_then(|raw| raw.parse::<bool>().ok()).unwrap_or_default(),
        }
    }
    ...
    pub fn int_param(&self, name: &String) -> Result<u64, String> {
        let v = self.test_instance_params
            .get(name)
            .cloned()
            .ok_or_else(|| format!("{} was not set", name))?;
        v.parse::<u64>()
            .map_err(|err| format!("{}={} value is not a valid u64", name, v))
    }
    ...
    pub fn float_param(&self, name: &String) -> Result<f64, String> {
        let v = self.test_instance_params
            ...
            .ok_or_else(|| format!("{} was not set", name))?;
        v.parse::<f64>()
            .map_err(|err| format!("{}={} value is not a valid f64", name, v))
    }
    ...
    pub fn bool_param(&self, name: &String) -> Result<bool, String> {
        let v = self.test_instance_params
            ...
            .ok_or_else(|| format!("{} was not set", name))?;
        v.parse::<bool>()
            .map_err(|err| format!("{}={} value is not a valid bool", name, v))
    }
}

fn unpack_params(packed: &str) -> HashMap<String, String> {
    let spltparams: Vec<String> = packed.split("|").collect();
